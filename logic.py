...
def pycoSAT(expr):
    ...
    symbol_dict = map_symbol_and_indices(clauses)
    # Convert Expr to integers
    clauses_int = expr_clauses_to_index_clauses(clauses, symbol_dict)
    ...
    model = index_model_to_expr_model(model_int, symbol_dict)
    
    return model

def map_symbol_and_indices(clauses):
    """
...
def expr_clauses_to_index_clauses(clauses, symbol_dict):
    """
    Convert each Expr in a list of clauses (CNF) into its corresponding index in
    the symbol_dict (see map_symbol_and_indices) 
    """
    clauses_int = []
    for c in clauses:
        c_disj = disjuncts(c)
...
def index_model_to_expr_model(model_int, symbol_dict):
    """
