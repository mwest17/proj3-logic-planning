...
class PacphysicsTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        if self.axiom_type == 'sensor':
            self.sensor_axioms = logicPlan.sensor_axioms
            self.successor_axioms = logicPlan.all_legal_successor_axioms
        elif self.axiom_type == 'slam':
            self.sensor_axioms = logicPlan.slam_sensor_axioms
            self.successor_axioms = logicPlan.slam_successor_axioms
        else:
    ...
    def solution(self, logicPlan):
        ...
        pacphysics_axioms = logicPlan.pacphysics_axioms(self.t, all_coords, non_outer_wall_coords, walls_list, self.sensor_axioms, self.successor_axioms)
...
class LocationSatisfiabilityTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for i, solution_i in enumerate(solution):
            ...
            solution_i = logicPlan.model_to_string(solution_i)
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        for i, solution_i in enumerate(solution):
            handle.write('{}: "{}"\n'.format(self.soln_labels[i], logicPlan.model_to_string(solution_i)))
        handle.close()
...
class PositionProblemTest(testClasses.TestCase):
    ...
    def solution(self, logicPlan):
        ...
        game_state = games[0].state
        return (game_state.is_win(), game_state.get_score(), pac.actions)
